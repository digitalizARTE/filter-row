...
Ext.ux.grid.FilterRow = Ext.extend(Ext.util.Observable, {
  ...
  onFieldChange: function() {
    ...
    * @returns {void} 
    */
    markNoData: function(add) {
        this.eachFilterColumn(function(col) {
            if (add && col.filter && col.filter.getFieldValue() != '') {
                col.filter.getField().addClass('filter-row-input-no-data');
            } else {
                col.filter.getField().removeClass('filter-row-input-no-data');
    }
  },
  ...
  refilter: function() {
        var store = this.grid.getStore();
        store.filterBy(this.getFilterFunction());
        //If you run it selects a single record
        var cnt = store.getCount();
        if (cnt == 0) {
            //TODO Show a sign that there are no records that match the filter
            this.markNoData(true);
        } else if (cnt == 1) {
            //Evito que el editor pierda el foco
            //this.grid.getView().focusRow(0);
            this.grid.getSelectionModel().selectFirstRow();
            this.markNoData(false);
        } else {
            this.markNoData(false);
        }
  },
